...
namespace eval system::utils {
	...
	if {$key == "Return"} {
	    set pattern ""
	    format_block $entry 0.0 end
	} elseif {$key == "braceright"} {
	}

	if {$editor($entry.addcontrols) == 1} {
	    if {$key == "f"} {
		if {[string trim [$entry get [$entry index insert-3chars] [$entry index insert]]] == "if"} {
		    $entry insert [$entry index insert] " ( ) {\n\n}"
		    $entry mark set insert [expr $current_position + 0.3]
		}
	    } elseif {$key == "h"} {
		if {[string trim [$entry get [$entry index insert-8chars] [$entry index insert]]] == "foreach"} {
		    $entry insert [$entry index insert] " ( ) {\n\n}"
		    $entry mark set insert [expr $current_position + 0.3]
		}
	    } elseif {$key == "y"} {
		if {[string trim [$entry get [$entry index insert-4chars] [$entry index insert]]] == "try"} {
		    $entry insert [$entry index insert] " {\n} catch (\\Exception \$e) {\n}"
		    $entry mark set insert [expr $current_position + 0.3]
		}
	    }
	...
	remove_tags $entry $current_line.0 $end
	...
	while {[set line [$entry get $current_line.0 [expr $current_line + 1].0 ]] != "" && $current_line <= $end_line } {
		...
		for {set x 0} {$x < $length} {incr x} {
			...
			if {[lindex [split $tag_end .] 0] > $current_line} {
			    set current_line [lindex [split $tag_end .] 0]
			    break
			} else {
			    continue
			}
		    ...
		    if {$word == "//"} {
			$entry tag add comment $current_line.[ expr $x - 2]  $current_line.end
			set x $length
			set word ""
		    } elseif {$word == "/*"} {
			...
			if {[ string first $char "{}()[]" ] != -1 } {
			    $entry tag add brace $current_line.$x $current_line.[expr $x+1]
			}
		    }
		}
	    }
	...
	set tag_end [ $entry search -regexp "$exp" $start_pos end ]

	if {$tag_end == "" } {
	    set tag_end [ $entry index end ]
	} else {
	    set tag_end [lindex [split $tag_end .] 0].[expr [lindex [ split $tag_end .] 1] + $delta_e ]
	}

	$entry tag add $tag [lindex [split $start_pos .] 0].[expr [lindex [split $start_pos .] 1] - $delta_s] $tag_end
	return $tag_end
    }
	...
	foreach tag [array names highlight_groups] {
		...
		if {$word == $w } {
		    ...
		    $entry tag add $tag $line.$start $line.$end
		}
	    }
	}
    }
